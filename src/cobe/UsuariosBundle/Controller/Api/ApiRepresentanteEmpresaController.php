                ...
                if(!in_array($id, $noModify)){
                    if($metadata->hasField($id)){
                        ...
                        if($metadata->isCollectionValuedAssociation($id)){
                            ...
                            if(empty($msgs)){
                                $set = 'set'.ucfirst($id);
                                if(method_exists($representanteEmpresa,$set)){
                                    //$collection = new ArrayCollection($collection);
                                    $representanteEmpresa->$set($collection);
                                }
                                $isModify = true;
                            }
                        }else{
                            ...
                            $accessor = PropertyAccess::createPropertyAccessor();
                            $dato_ = $accessor->getValue($representanteEmpresa, $id);
                            if($dato && (!$dato_ || (is_object($dato_) && method_exists($dato_,'getId') && $dato_->getId() !== $dato))){
                                ...
                                if($association && $association->getId()){
                                    $accessor->setValue($representanteEmpresa, $id, $association);
                                    $isModify = true;
                                }
                            }
                        }
